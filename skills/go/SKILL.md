...
The workflow loops review → replan → re-review for up to 5 rounds (the cap check fires before the replan, so at most 4 replans) and stops early when a round's CRITICAL set repeats. When `args.answers` is present it first runs an optional **apply-answers step** (`replan:answers`, a replanner that lands the user's answers in PLAN.md before any review can approve; it consumes a `### Round n` label but not a review round). It returns `{ feature, status, rounds, findings, questions?, examined?, history, nextRoundOffset, blockedBy? }` — `nextRoundOffset` is the total number of `### Round n` labels consumed across all invocations so far (the answers step included), and `blockedBy` (`reviewer` | `replanner` | `answers`) names which agent went missing on `BLOCKED`. Agent output stays inside the workflow — you receive only this structured result.
...
[If `history` is non-empty, one line per entry:]
- Round {n}: {reviewStatus}, {criticals} critical
[An entry with `step: 'answers'` renders instead as:]
- Answers applied: {changes.length} change(s)
```
...
- **`NEEDS_INPUT`** — split on `--headless`:
  - **Interactive (no `--headless`)** — ask each entry in `questions` via AskUserQuestion (one question per entry, using its `options`; the automatic Other option covers anything else). Then RE-INVOKE the same workflow with `args: { feature: "{name}", answers: "<Q/A transcript>", roundOffset: <the result's nextRoundOffset>, findings: <the result's findings>, maxPlanRounds: {knobs.maxPlanRounds} }` and re-branch on the new status. `nextRoundOffset` is the total number of `### Round n` labels consumed so far across all invocations — it counts an apply-answers step, which summing `rounds` would miss; when the field is absent (an older workflow), fall back to the summed `rounds`. It is what keeps the replanner's `### Round {n}` headings unique across re-invocations — without it a second run restarts at `### Round 1` and collides with the first run's subsection, which the replanner is forbidden to rewrite. `findings` are the escalated CRITICALs the re-invoked workflow applies the answers against: it runs them through a `replan:answers` step before its first review, so an answer can no longer be dropped by an approving review. Do **not** use `resumeFromRunId`. Cap this at 2 re-invocations; if a third `NEEDS_INPUT` arrives, report it and stop.
  - **Headless** — do NOT call AskUserQuestion. Write `.planning/features/{name}/QUESTIONS.md` in the format specified in **`ship/docs/headless.md` §6**, recording `roundOffset` as the result's `nextRoundOffset` (the total `### Round n` labels consumed so far across all invocations; fall back to the summed `rounds` when the field is absent). Leave CONTEXT.md at `planned` and terminate as `needs-input` with `questions_file` set.
...
- **`BLOCKED`** — leave CONTEXT.md `status: planned`. Split on `blockedBy` (when the field is absent, read `reason` to tell which agent went missing):
  - **`reviewer`** — before reporting, run the **scratch fallback**. The reviewer writes its record incrementally, so a run that died after finishing its review still left that review on disk. Run `git hash-object .planning/features/{name}/PLAN.md`, then Read `.planning/features/{name}/.review-scratch/plan-round-{rounds}.json` (`rounds` from the result — the reviewer names its record by bare loop round). If the file exists, parses as JSON, its `plan_hash` equals that hash, and `complete` is `true`, adopt its `findings` as this round's review:
    - **Zero CRITICALs** → take the `APPROVED` branch exactly as if the workflow had returned it: outcome block heading `### Outcome — APPROVED (adopted from scratch record)`, CONTEXT.md `status: plan-verified`, continue to section 3.
    - **CRITICALs present** → copy the record over `.planning/features/{name}/.review-scratch/plan-round-1.json` (the fresh run's round-1 reviewer adopts it through its own salvage check, which runs before any exploration; `plan_hash` keeps the copy safe), then re-invoke the workflow **once** — `plan.workflow.js` with `args: { feature: "{name}", roundOffset: <the result's nextRoundOffset>, maxPlanRounds: {knobs.maxPlanRounds} }` (plus `answers` and `findings` if this invocation had them) and re-branch on the new status. A second `BLOCKED` reports as below with no further re-invocation.
    - **Missing, unparseable, hash-mismatched, or incomplete record** → report as before: an agent produced no result after retry (a plan is never approved without a completed review) and the run stopped; suggest `/ship:plan-verify {name}` to review once manually. Stop.
  - **`replanner` / `answers`** — the report must say PLAN.md **may already be partly or fully revised**: the replanner edits before it writes its subsection, so a lost result is not proof the plan is untouched. Point at `.planning/features/{name}/.review-scratch/replan-round-{n}.json` (the workflow's `reason` names it) for what landed, and recommend re-running `/ship:go {name}` — the retry salvages the record and resumes from the first pending finding. Do **not** send the user to a manual review: reviewing a half-revised plan is the wrong remedy. Stop.
...
**Scratch cleanup — after the branch, never before it.** The reviewer scratch fallback above reads `plan-round-{rounds}.json`, so this step runs only once the `BLOCKED` branch has finished (adopted, re-invoked, or reported) — deleting first would destroy the record the fallback exists to read. On every terminal outcome except `NEEDS_INPUT` (whose re-invocation still needs them — including a headless park, which is terminal for this process but leaves the loop live, so the files are kept exactly as on interactive `NEEDS_INPUT`) and except a `BLOCKED` whose `blockedBy` is `replanner` or `answers` (the report just pointed the user at `replan-round-{n}.json`, and the recommended `/ship:go` re-run salvages it), delete `.planning/features/{name}/.review-scratch/plan-round-*.json` and `.planning/features/{name}/.review-scratch/replan-round-*.json` — the plan reviewers' and replanner's crash-recovery cache for the loop that just ended. Leaving them is not dangerous (each is fingerprinted with the PLAN.md hash it was written against, so a salvage rejects one from a different plan) but they serve no further purpose. On `APPROVED`, section 6 clears the whole directory anyway.
