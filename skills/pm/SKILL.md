...
1. Read `${CLAUDE_PLUGIN_ROOT}/skills/pm-state/SKILL.md` for the six state-file formats (ROADMAP.md, STATUS.md, DECISIONS.md, CONVENTIONS.md, LEDGER.md, dashboard.html), the status mapping table, the PM:NEXT and PM:PRIORITY rules, and the hard rules.
...
4. For every verb and for free-text questions, invoke the Agent tool with `subagent_type: "ship:ship-pm"`, passing the matching verb brief below verbatim plus the raw `$ARGUMENTS`. The agent owns the writes. Do not read state into this conversation yourself, and do not re-derive the agent's findings — relay its report.
   ...
   > **status** — Reconstruct the true state; do not recite STATUS.md back. Check the tracking files against git, feature frontmatter, and whether each shipped feature has a VERIFY.md recording a result. Report the delta between the files and the repo, then fix the files. Summarize `LEDGER.md` every run — how many features it records, the verdict distribution, and any pattern worth naming, calling out every `Verify: none` row as verification debt. Run `node "${CLAUDE_PLUGIN_ROOT}/ship/pm-update.cjs" --debt` (one proposed backlog row per ledger row carrying verification debt, each with a `Source` naming that row) and `--lint` (state-file decay: over-cap `Source` cells, undeclared STATUS.md frontmatter keys, narrative above the first section); both write nothing, and both are surfaced as **proposals the user accepts or declines** — never written as backlog rows on your own, the same propose-never-write discipline that already governs Priority. End with what most deserves to happen next.

   > **groom** — Re-check every backlog item still applies. Verify each carries a traceable Source and drop or flag any that does not. Re-prioritise by the P0–P3 key, re-size (S/M/L/XL), and make dependencies explicit. Run `node "${CLAUDE_PLUGIN_ROOT}/ship/pm-update.cjs" --evidence` and report every item whose `derived` differs from `recorded` as a proposed change with its evidence (`Session tokens: P2→P1 — blocks 3 items, 1 in flight`), and every `needsEvidence: true` item as a request for the missing `Blast radius` / `Confidence` value. You propose and argue; you **never write the Priority cell** — the user decides. Also run `node "${CLAUDE_PLUGIN_ROOT}/ship/pm-update.cjs" --debt` and `--lint`: surface each debt proposal (with its ledger-row `Source`) and each lint finding as a **proposal the user accepts or declines**, and write a `Kind: debt` backlog row only for the ones they accept. Report what moved and why.
...
5. Delegate a read-mostly brief to the agent:
   ...
   - **Awaiting merge** — every backlog row recorded `awaiting-merge` on its own line: the feature is archived and verified but its VERIFY.md `**Head:**` commit is not yet an ancestor of the base branch. Deliberately not reported as in flight — nobody is working on it — and deliberately not reported as done. The remedy is merging the PR, after which the next reconcile flips the row itself.
   - **Pending PM handoffs** — one line per `pendingHandoffs` entry from the same sweep: feature, lane, and how many edits wait. This is work only this layer can do, so it belongs in every brief until it is applied. An entry marked `unparseable: true` is reported by **path and reason** — never counted as an absent handoff and never as an applied one. End the section with `/ship:pm apply` when any are pending.
   - **Ledger summary** — from `.project-manager/LEDGER.md`: how many features it records, the verdict distribution, and any pattern worth naming (a `Verify: none` row, repeat unresolved carries, a rising fix-round count). It appears in every brief; the counts are mechanical, naming the pattern is the agent's job.
   - Top 1–3 priorities
