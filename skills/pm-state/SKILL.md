...
- The `Lane` column arrives only via a confirmed `/ship:pm-sync` reconcile — the same growth pattern that took v5.3.0 tables from 5 to 7 columns. A 7-column table without it stays valid indefinitely. `Blast radius`, `Confidence`, `First seen`, and `Kind` arrive the same way: `pm-update.cjs` never widens a table itself, so a narrower table simply reads those columns as `unknown` and receives no promotion — and a table with no `Kind` column reconciles every row as `work`, exactly as it always did.
