...
- **`findFeatures()` guards the slug — a slug is a name, not a path.** A slug that is empty after trimming, whitespace-only, or contains `/`, `\`, or `..` anywhere is rejected before either `listSlugs` call runs: the result carries `features: {}` and `warning` reads `ignored invalid slug '{slug}' — a slug is one path segment`, joined with any other warnings as before; `cwd`, `cwdRoot`, `mainRoot`, and `worktrees` are still computed and returned. Rejected with a warning rather than coerced or thrown: `path.basename()`-ing `../../../outside` into `outside` could silently resolve a different, real feature with no signal, and a throw or non-zero exit would break the pipe-reading skills — the CLI still exits 0 with one line of JSON, and the helper still has no `process.exit()`. The guard tests presence, not truthiness, so `''` and `'   '` are rejected instead of quietly meaning "no filter"; the one visible behaviour change is that an empty-string CLI argument (`node ship/find-features.cjs ''`) is now rejected rather than treated as unfiltered. No skill passed one. An omitted, `null`, or `undefined` slug is the unfiltered scan exactly as before, and a legitimate slug follows the previous code path byte-for-byte.
- **`/ship:resume` calls the helper unfiltered and looks the name up on the map.** Step 1 shows one command line with no `{name}` argument, and step 2 picks `features[name]` by exact key. The not-found prose now has two cases: a name was given and no key matched → "`{name}` was not found in any checkout or the archive" plus a listing of every entry that does exist as `{slug} [{status} · {location}]`, and no `/ship:start` suggestion — a near-miss name is far more likely a typo than new work; `features` is genuinely empty → no features exist anywhere, suggest `/ship:start`, still only after the helper has looked across every worktree. `tests/slug-input-hygiene.test.js` pins both: fixture-backed guard tests run the helper and the CLI against a real repo beside an `outside/CONTEXT.md` carrying `status: leaked-status` and assert the string never appears in a result, and a doctrine block fails if resume's helper line regains an argument or `/ship:start` becomes reachable from the named-miss branch.
...
- **The replanner writes a scratch record and resumes from it.** `.review-scratch/replan-round-{n}.json` (`n` is the `### Round n` label) is written **before the first edit** with every finding at `pending`, rewritten after each finding is revised, disproved, or escalated, and stamped `complete` once the round subsection lands. The subsection was the old salvage key, and it is written last — so it was absent for exactly the runs that needed salvaging. A retry now reads the record, treats `revised` / `disproved` / `escalated` findings as done (their edits are already in PLAN.md; applying them again would double-apply), and resumes from the first `pending` one. The record's `plan_hash` is taken before the first edit, so it tells an untouched plan from a partly revised one; staleness is judged on `round` and the finding key set (`task_id||file`) rather than hash equality, because a successful replan necessarily changes the hash. The workflow's replan salvage prompt names the record instead of the subsection.
- **An apply-answers step.** When `/ship:go` re-invokes `plan.workflow.js` with `args.answers`, the workflow runs one `replan:answers` replanner call **before** `plan-review:r1`, carrying the answers and the escalated findings, so the user's ruling is in PLAN.md before any review can approve. It consumes a `### Round n` label but not a review round, appears in `history` as `{ round: 0, step: 'answers', reviewStatus: 'ANSWERS_APPLIED' }`, and the round-1 review that follows is asked to confirm the answers landed. The in-loop replan prompts still carry the transcript, exactly as before.
- **A scratch fallback in the go skill and in `/ship:plan-verify`.** On a `BLOCKED` whose reviewer went missing, the go skill reads `.review-scratch/plan-round-{rounds}.json` and, when the record is `complete` and its `plan_hash` equals `git hash-object PLAN.md`, adopts it as that round's review: zero CRITICALs approves the plan; otherwise the record is copied over `plan-round-1.json` and the workflow re-invoked **once**, so the fresh round-1 reviewer adopts it through its own salvage check instead of re-reviewing from scratch. Scratch cleanup is ordered after the fallback, never before it. `/ship:plan-verify` applies the same hash-matched adoption when its second launch fails, recording `**Source:** adopted from the reviewer's scratch record` under the verdict; Bash joins its `allowed-tools` for `git hash-object` and nothing else.
- **`/ship:plan` replan mode.** When PLAN.md exists with a `## Plan Review` whose latest verdict is not approved — the last `**Status:**` is `NEEDS-REVISION`, or the last outcome is `STUCK`, `UNRESOLVED`, or `BLOCKED` — the open CRITICALs become the required inputs to the run. Each is verified against the code (a disproved finding is recorded, not dropped), the tasks it names are revised in place with their ids kept, the whole `## Plan Review` section is preserved, and a `### Round n — /ship:plan` subsection is appended. Anything else plans from scratch exactly as before.
- **`blockedBy` and `nextRoundOffset` on workflow results.** Every `BLOCKED` names the missing agent (`reviewer` | `replanner` | `answers`) so the go skill routes the fallback without parsing `reason`. Every result carries `nextRoundOffset` — the total number of `### Round n` labels consumed across all invocations, the answers step included — which the go skill threads back as the next `roundOffset` and into QUESTIONS.md frontmatter; summing `rounds`, the old derivation, would miss the answers step and collide the next replan's heading with one the replanner is forbidden to rewrite. The summed form remains the fallback when the field is absent.

### Changed

- **`ship-replanner` raised from `maxTurns: 30` to `60`.** 5.11.0 kept it at 30 on the grounds that there was "no evidence it has ever reached it". The audit falsified that: two `BLOCKED` runs carrying 7 and 8 CRITICALs each verified every finding against the code, rewrote most of the plan, and were cut off before the round subsection — the same silent harness cut the reviewers were raised for. The cap now matches theirs.
- **The lost-replan `BLOCKED` report tells the truth about the plan.** It used to send the user to `/ship:plan-verify` as if PLAN.md were untouched. The replanner edits before it writes its subsection, so a lost result is not proof the plan is unrevised — the report now says PLAN.md **may already be partly or fully revised**, points at `replan-round-{n}.json` for what landed, and recommends re-running `/ship:go`, whose retry salvages the record. Reviewing a half-revised plan is the wrong remedy, and the report no longer suggests it.

### Fixed

- **Both salvage checks said "and stop".** The plan reviewer's and the replanner's salvage instructions ended a successful salvage with "report … and stop", which contradicts the rule that calling `StructuredOutput` is the agent's final action — so a salvage that found its record reported to nobody. This matches the observed shape exactly: a complete scratch record written, then the agent exiting without emitting it, twice in a row. Both checks now end on the `StructuredOutput` call, and say why: a salvage that reports without it is itself a lost result.
- **A `NEEDS_INPUT` answer could be dropped silently.** The re-invoked workflow interpolated the transcript into the replan prompts only, so a round-1 review that approved the (unchanged) plan ended the loop with the user's ruling never applied — one feature was approved and built that way. The apply-answers step lands the ruling before the first review can run.
...
- **The ledger reads `**Verdict: PASS.** Criterion 8 holds.` as a PASS.** ([#30](https://github.com/dilhanz/ship/issues/30)) The inline `**Verdict: X**` shape captures to end of line, so the closing asterisks landed *inside* the first token — where the decoration strip, which only trims the ends of the captured text, could not reach them. `extractVerdict` returned `unknown` for the natural way a verifier opens a report: a verdict with prose after it. It failed toward `unknown` rather than a wrong verdict, the safe direction, but it manufactured phantom verification debt — a feature that passed every gate recorded as unverified, and a `--debt` run proposing a backlog row for debt that does not exist. The token-level strip now takes trailing `*` and `` ` `` alongside sentence punctuation, which additionally reads a backtick-wrapped verdict. Re-harvesting an affected row records the real verdict.
...
- **`/ship:finish` states that archiving relocates the record for everyone.** ([#31](https://github.com/dilhanz/ship/issues/31)) The archive `mv` lands on `main` out of band while every open branch still carries `.planning/features/{slug}/`, so a follow-up PR amending that record — marking a carried finding resolved in VERIFY.md, say — edits a path `main` no longer has. Local git resolves it as the rename it is; GitHub has been observed to report `mergeable: CONFLICTING` naming no file even for an isolated, near-pure-rename archive commit, so rename detection is not something to rely on. The skill now says so, prescribes the two-minute fix (merge `main` into the branch locally, confirm one copy survives at the archive path, push the merge), tells the finish report to surface it, and notes that anything outside `.planning/` hard-coding a feature path breaks at the same moment. Documentation only: keying `done` on the frontmatter stamp instead of directory presence would remove the collision class outright, but it breaks archive-wins precedence, `archiveMergeStatus`, `lane-sweep.cjs` and every roadmap source citing an archive path — not worth its cost on one occurrence.
