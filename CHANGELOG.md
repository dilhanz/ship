...
- **`/ship:pm-sync` bootstraps and grows to a 12-column backlog table**, adding `Kind` to the enriched header. This is the arrival path the column needs: a `debt` row cannot be recorded in a table that has no cell to hold it, and a row written without the cell reconciles as `work` and auto-closes off the feature's archive — the exact blind spot `Kind` exists to close. `Kind` is **authored**, like `Blast radius` and `Confidence`: growth fills it with `work` for every existing row without asking, since that is what an absent column already means, so a widened table reconciles byte-for-byte as it did before. A row is promoted to `debt` only by accepting a `--debt` proposal, never while widening. Declining growth is still fully supported — the cost is simply that the table cannot hold a `debt` row, which is correct rather than degraded.
