...
- **Verification debt:** `node "${CLAUDE_PLUGIN_ROOT}/ship/pm-update.cjs" --debt` prints one entry per LEDGER.md row whose `Verify` cell is `none`, `unknown`, `FAIL`, or `INCONCLUSIVE` — `{item, status, priority, size, dependsOn, source, shipFeature, kind, verify, outcome, reason}`, in the ledger's own row order — and **writes nothing at all**: no roadmap edit, no stamp, no dashboard, no ledger append. Rows whose `Outcome` is `abandoned`, `superseded`, or `umbrella` are excluded: work that was never meant to ship is not verification debt. Every entry is a **proposed** backlog row carrying `kind: debt` and a `Source` naming the ledger row it came from — you surface it and the user accepts or declines; you never write one on your own, exactly as with `--evidence` and Priority. An absent LEDGER.md or `.project-manager/` prints `[]` and exits 0.
- **State-file decay:** `node "${CLAUDE_PLUGIN_ROOT}/ship/pm-update.cjs" --lint` prints `{sourceOverCap, statusUndeclaredKeys, statusNarrativeBeforeSections}` — backlog `Source` cells over the 240-character cap (with `item`, `milestone`, `length`), STATUS.md frontmatter keys other than `updated`, and non-blank lines between the frontmatter and the first `## ` section (the `# {project} — Status` H1 excepted). It **writes nothing** and never fails: an absent file degrades to an empty array for its axis and exits 0. The findings are proposals too — a long `Source` is spilled to a DECISIONS.md entry or a `file:line` with the user's agreement, never truncated behind their back.
...
Run `--debt` and `--lint` every `status`. `--debt` turns the ledger summary into named, actionable proposals — one per row carrying verification debt, each with a ledger-row `Source` — instead of a count you have to re-derive; `--lint` catches the state files decaying underneath you (an over-cap `Source` that has become an essay, an undeclared STATUS.md frontmatter key, narrative stranded above the first section). Report both as **proposals the user accepts or declines**. Neither writes anything, and neither authorises you to: a `Kind: debt` row reaches the backlog only when the user accepts it.

Report every backlog row recorded **`awaiting-merge`** as its own line, not folded into in flight or recently shipped: the feature is archived and verified, but its VERIFY.md `**Head:**` commit is not yet an ancestor of the base branch. Nobody is working on it and it is not on `main` — the remedy is merging the PR, and the next reconcile flips the row without your help.
...
0. If the entry is marked `unparseable: true`, **do not apply it blind.** The sweep detects a handoff by filename and parses afterwards, so this is a real file on disk that could not be read as a handoff. Report it by path and `reason` (`unreadable: {code}`, `no frontmatter block`, `frontmatter missing feature:`), name the lane that wrote it, and show the user the file. It is never counted as applied and never counted as absent. It also blocks a `handover` prune of that lane, which is deliberate — the remedy is to fix or delete the file, and that is the user's call, not yours.
